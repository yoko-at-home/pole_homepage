...
export const Breadcrumb: FC<BreadcrumbProps> = ({ items = [], showHome = true }) => {
  ...
  const getPageLabel = (segment: string): string => {
      ...
      {breadcrumbItems.map((item, index) => {
        return (
          <div key={`breadcrumb-${item.label}-${index}`} className="flex items-center">
            {index > 0 && <ChevronRight className="mx-1 h-4 w-4 text-gray-400" aria-hidden="true" />}
            {item.href ? (
              <Link href={item.href} className="flex items-center hover:text-blue-600 hover:underline">
